...
Delta 到事件的映射**不是**一个转换阶段 —— 它发生在 `HostedWebSearchStreamRunner` 内部（见[流式管道](./streaming-pipeline.md)），该 runner 生产转换链消费的 `ResponseStreamEvent` 流。

## 转换链顺序

[src/responses/stream-pipeline.ts:25](https://github.com/Ahoo-Wang/GodeX/blob/main/src/responses/stream-pipeline.ts#L25) 中的 `StreamPipeline.stream()` 方法组装该链。每个阶段通过 [src/responses/stream-transforms/stream-utils.ts:6](https://github.com/Ahoo-Wang/GodeX/blob/main/src/responses/stream-transforms/stream-utils.ts#L6) 中的 `pipeTransform` 辅助函数连接，该函数将每个转换器包装在标准的 `TransformStream` 中。事件流本身由 `HostedWebSearchStreamRunner` ([web-search/stream-runner.ts:60](https://github.com/Ahoo-Wang/GodeX/blob/main/src/responses/web-search/stream-runner.ts#L60)) 生产。

```mermaid
flowchart LR
    subgraph Production ["事件生产 (HostedWebSearchStreamRunner)"]
        style Production fill:#161b22,stroke:#30363d,color:#e6edf3

        Upstream["Provider<br>Stream"]
        T1["TraceTransformer<br>(raw)"]
        Map["mapProviderDeltasToEvents<br>+ web search 循环"]
    end

    subgraph Chain ["转换链 (StreamPipeline.stream)"]
...
管道在 [src/responses/stream-pipeline.ts:31](https://github.com/Ahoo-Wang/GodeX/blob/main/src/responses/stream-pipeline.ts#L31) 处组装，每个 `pipeTransform` 调用将前一个 `ReadableStream` 链接到下一个转换器。
...
`TraceTransformer` 是一个直通转换器，将每个流事件记录到追踪子系统。它在路径中出现**两次** -- 一次在 `HostedWebSearchStreamRunner` 内部（记录原始上游事件），一次在转换链验证之后（记录已转换事件）。
...
[src/responses/stream-transforms/compatibility-log-transformer.ts:29](https://github.com/Ahoo-Wang/GodeX/blob/main/src/responses/stream-transforms/compatibility-log-transformer.ts#L29) 处的 `onFlush` 回退确保即使流在没有终止事件的情况下关闭也能记录诊断信息。日志条目的严重级别与最严重的诊断相匹配 -- 错误为 `error` 级别，警告为 `warn` 级别，信息性诊断为 `info` 级别。
...
一旦终止事件被重写，所有后续事件将被静默丢弃（`if (this.rewrittenTerminal) return`，位于 [src/responses/stream-transforms/response-output-contract-validation-transformer.ts:27](https://github.com/Ahoo-Wang/GodeX/blob/main/src/responses/stream-transforms/response-output-contract-validation-transformer.ts#L27)）。这可以防止合约失败后部分输出泄漏到客户端。
...
- [架构概览](./architecture-overview.md) -- 流转换在系统中的上下文
...
9. [src/responses/stream-pipeline.ts:31-66](https://github.com/Ahoo-Wang/GodeX/blob/main/src/responses/stream-pipeline.ts#L31) -- 管道组装
