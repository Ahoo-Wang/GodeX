---
title: 桥接兼容性规划
description: GodeX 如何在构建 Chat Completions 请求之前规划 Responses API 与各上游提供方之间的功能兼容性。
---

# 桥接兼容性规划

并非每个上游提供方都支持 OpenAI Responses API 的每一项功能。GodeX 必须在发送任何请求之前确定：哪些功能是原生支持的，哪些可以降级到相近的替代方案，哪些必须被静默忽略，哪些是硬性阻断。兼容性子系统以确定性方式做出这些决策，并为每个决策记录诊断信息，使运维人员能够完全了解某个请求为何以特定方式构建。

## 概览

| 概念 | 类型 | 用途 |
|------|------|------|
| `ProviderCapabilities` | 接口 | 声明提供方支持的功能（参数、工具、格式、推理） |
| `CompatibilityPlan` | 接口 | 保存单个请求的所有决策 |
| `CompatibilityDecision` | 接口 | 单个决策：动作、原因、实际值 |
...
每个兼容性决策精确地解析为以下四种动作之一：

| 动作 | 含义 | 诊断严重程度 |
|------|------|--------------|
| `supported` | 功能原生支持；原样转发 | --（不产生诊断） |
| `degraded` | 功能映射到相近的替代方案；行为可能有所不同 | `warn` |
| `ignored` | 功能被静默丢弃；请求继续但不包含该功能 | `warn` |
| `rejected` | 功能是硬性阻断；请求以 `BridgeError` 中止 | `error` |

## 能力域

提供方通过 [`ProviderCapabilities`](https://github.com/Ahoo-Wang/GodeX/blob/main/src/bridge/compatibility/compatibility-plan.ts#L29-L36) 接口声明其能力：
...
| 推理 | `reasoning.effort` | `"none" / "boolean" / "native"` | 是否以及如何转发推理强度 |
| 流式 | `streaming.usage` | `boolean` | SSE 流是否包含使用量数据 |
...
`planTools` 函数根据提供方的 `ToolPlanningProfile` 评估每个工具声明：
...
| `required` | 两者均不支持 | rejected（错误） |
| 具名 function/custom | 匹配的声明 + 提供方类型 | supported 或 degraded |
| 具名 function/custom | 无匹配声明 | rejected（错误） |

## 响应格式降级

当提供方不原生支持 `json_schema` 但支持 `json_object` 时，输出合约会被降级。系统消息中会注入一条合成指令，包含原始 JSON Schema、验证规则以及 GodeX 将验证输出的说明：

| 降级方式 | 提供方 `response_format` | 合成指令 | 后验证 |
|----------|------------------------|----------|--------|
| `json_schema` 到 `json_object` | `{ type: "json_object" }` | Schema 名称、描述和完整 JSON Schema | `strict` 时 `requiresValidJson: true` |
...
不同提供方处理推理强度的方式不同。`reasoning.effort` 能力决定映射方式：

| 能力模式 | 行为 |
|----------|------|
| `native` | 直接转发 `reasoning_effort`（例如 `low`、`medium`、`high`） |
| `boolean` | 映射为 `thinking.type`：`"none"` 变为 `"disabled"`，其他变为 `"enabled"` |
| `none` | 推理强度被静默忽略；不转发任何参数 |

## 诊断代码

每个非 supported 的决策都会产生一个带有机器可读代码的诊断：

| 代码 | 触发条件 |
|------|----------|
| `bridge.param.ignored` | GodeX 自有参数（metadata、conversation、background）或不支持的工具类型 |
| `bridge.param.degraded` | 功能降级（json_schema 到 json_object、tool_choice required 到 auto） |
| `bridge.param.unsupported` | 硬性拒绝不支持的功能 |
| `bridge.tool.compatibility` | 工具声明不被原生支持（动作因决策而异） |
...
- **[响应重建](./response-reconstruction.md)**：响应后验证如何使用输出合约
...
- [src/bridge/output/output-contract.ts:1-75](https://github.com/Ahoo-Wang/GodeX/blob/main/src/bridge/output/output-contract.ts#L1-L75) -- `planOutputContract` 和 JSON Schema 降级及合成指令
