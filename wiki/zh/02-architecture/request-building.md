...
GodeX 桥接的核心目的是将 OpenAI **Responses API** 的 `ResponseCreateRequest` 转换为 **Chat Completions API** 的 `ChatCompletionCreateRequest`。这不是简单的字段映射——它需要兼容性规划、工具声明翻译、输出合约协商、跨多种项类型的输入标准化，以及为满足 Chat Completions 约束而进行的消息合并。`buildChatCompletionRequest` 函数以固定序列编排整个管道。
...
| 5 | `buildChatMessages` | 合并了助手轮次的 `ChatCompletionMessageParam[]` | [message-builder.ts](https://github.com/Ahoo-Wang/GodeX/blob/main/src/bridge/request/message-builder.ts#L7-L56) |
...
Responses API 支持许多在 Chat Completions API 中没有直接对应项的输入类型。`normalizeCurrentInput` 和 `normalizeResponseItems` 将每种项类型转换为一个或多个 `ChatCompletionMessageParam` 条目：

| Responses 项类型 | Chat Completions 映射 |
|-----------------|----------------------|
| `message`（角色：system/user/assistant/developer） | `{ role, content }`（developer 映射为 system） |
| 带有 `instructions` 的 `message` | 前置为 `{ role: "system", content: instructions }` |
| `reasoning` | 追加到下一个助手消息的 `reasoning_content` |
| `function_call` | `{ role: "assistant", tool_calls: [{ id, function }] }` |
| `function_call_output` | `{ role: "tool", tool_call_id, content }` |
| `shell_call` | `{ role: "assistant", tool_calls: [...] }`，带 JSON 序列化的操作 |
| `shell_call_output` | `{ role: "tool", tool_call_id, content }`，带格式化输出 |
| `local_shell_call` | `{ role: "assistant", tool_calls: [...] }`，带 JSON 序列化的操作 |
| `local_shell_call_output` | `{ role: "tool", tool_call_id, content }` |
| `apply_patch_call` | `{ role: "assistant", tool_calls: [...] }`，带 JSON 序列化的操作 |
| `apply_patch_call_output` | `{ role: "tool", tool_call_id, content }` |
| `custom_tool_call` | `{ role: "assistant", tool_calls: [...] }`，带 `{ input }` 载荷 |
| `custom_tool_call_output` | `{ role: "tool", tool_call_id, content }` |

### 工具名称映射

每个工具调用使用 `ToolPlan.declarations` 中的提供方侧名称。标准化器在声明中查找请求的名称和类型，以找到映射后的 `providerName`：
...
Chat Completions API 不允许包含独立工具调用的连续助手消息。`buildChatMessages` 使用四条规则合并相邻的助手消息：
...
`applyRequestOptions` 有条件地将每个 Responses API 参数映射到 Chat Completions 等效项。只有当提供方的 `capabilities.parameters.supported` 集合包含该参数时才会转发：

| Responses 参数 | Chat Completions 字段 | 条件 |
|---------------|---------------------|------|
| `stream: true` | `request.stream = true` + `stream_options.include_usage` | 提供方支持 `stream`；usage 仅在 `streaming.usage` 时包含 |
| `temperature` | `request.temperature` | 提供方支持 `temperature` |
| `top_p` | `request.top_p` | 提供方支持 `top_p` |
| `max_output_tokens` | `request.max_tokens` | 提供方支持 `max_output_tokens` |
| `reasoning.effort` | `request.reasoning_effort` 或 `request.thinking` | 提供方能力模式：`native`、`boolean` 或 `none` |
| `safety_identifier` | `request.user_id` | 提供方支持 `safety_identifier` |
| `user` | `request.user_id` | 提供方支持 `user`（无 safety_identifier 时的后备） |
...
| 字段 | 类型 | 用途 |
|------|------|------|
| `request` | `ChatCompletionCreateRequest` | 要发送到上游的最终请求 |
...
- **[响应重建](./response-reconstruction.md)**：上游返回后响应如何被映射回来
