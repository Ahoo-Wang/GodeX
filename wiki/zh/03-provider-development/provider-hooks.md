...
GodeX 的桥接运行时使用一种内部协议，但每个上游提供商都有各自的特点 -- DeepSeek 使用原生的 `reasoning_effort` 参数和 `thinking` 对象，Zhipu 支持 `web_search` 和 `file_search` 工具类型，MiniMax 将 `max_tokens` 重映射为 `max_completion_tokens`。`ProviderHooks` 是每个提供商注入其自身规范化逻辑的扩展点。通过保持钩子可选且与提供商规约同目录，GodeX 避免了庞大的适配器层，让每个提供商拥有自己的转换逻辑。
...
|---|---|---|---|
| 推理努力 | `native`（high/max） | `boolean`（enabled/disabled） | `none` |
| 最大工具数 | 128 | 128 | 128 |
