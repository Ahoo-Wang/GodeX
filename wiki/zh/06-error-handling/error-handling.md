---
title: 错误处理
description: GodeX 的类型化错误层次结构，包含领域特定的子类、结构化错误码和从提供商上游到桥接层再到客户端响应的 HTTP 传播机制。
---

# 错误处理

GodeX 将每种失败建模为类型化的、领域范围限定的对象，使调用者、运维人员和日志聚合管道都使用统一的语言。系统没有在代码库中散布字符串消息，而是定义了一个抽象基类——`GodeXError`——以及四个与架构层一一对应的具体子类。每个错误都携带机器可读的错误码、HTTP 状态码、结构化上下文包和毫秒精度的时间戳，便于从上游提供商超时到返回给客户端的 JSON 响应体进行端到端的故障追踪。

## 概览

| 方面 | 详情 |
|---|---|
| 基类 | `GodeXError` -- 抽象类，继承 `Error` |
| 领域 | `server`、`bridge`、`provider`、`session` |
| 关键字段 | `domain`、`code`、`status`、`context`、`timestamp` |
| 日志记录 | `toLogEntry()` 生成纯对象快照 |
...
| 错误码 | 集中定义于 [src/error/codes.ts](https://github.com/Ahoo-Wang/GodeX/blob/main/src/error/codes.ts) |
...
抽象 `GodeXError` 类定义于 [src/error/godex-error.ts:2-35](https://github.com/Ahoo-Wang/GodeX/blob/main/src/error/godex-error.ts#L2)。每个子类声明固定的 `domain` 字符串，并将构造函数参数转发给基类，由基类规范化 `status` 和 `context` 的默认值。

## 错误领域与错误码

错误码遵循 `<domain>.<subdomain>.<specific>` 的命名模式，并以字符串常量形式从 [src/error/codes.ts:1-52](https://github.com/Ahoo-Wang/GodeX/blob/main/src/error/codes.ts#L1) 导出。
...
在 OpenAI Responses 模式与提供商原生格式之间转换时抛出。

| 错误码 | HTTP | 触发时机 |
|---|---|---|
| `bridge.request.unsupported_parameter` | 400 | 参数在提供商侧无对应项 |
| `bridge.request.tool_skipped` | 400 | 工具不受提供商支持 |
...
| `bridge.response.invalid_output_format` | 400 | 提供商输出无法映射回来 |
...
[来源：src/error/codes.ts:33-36](https://github.com/Ahoo-Wang/GodeX/blob/main/src/error/codes.ts#L33)、[src/server/errors.ts:20-44](https://github.com/Ahoo-Wang/GodeX/blob/main/src/server/errors.ts#L20)
...
| 错误码 | HTTP | 触发时机 |
|---|---|---|
| `session.chain.not_found` | 404 | 之前的响应 ID 不存在 |
| `session.chain.cycle_detected` | 400 | 检测到循环链引用 |
| `session.chain.depth_exceeded` | 400 | 链超过配置的最大深度 |
...
sequenceDiagram
    ...
    alt 上游失败
        ...
        Server-->>Client: 映射后的 HTTP (429/502/422)
    end
...
路由级错误处理器位于 [src/server/routes/responses/error-handler.ts:12-50](https://github.com/Ahoo-Wang/GodeX/blob/main/src/server/routes/responses/error-handler.ts#L12)，按优先级顺序分发错误：

1. **ProviderError** -- 以 `error` 级别记录日志，通过 `providerErrorToHttp` 映射。
2. **其他 GodeXError** -- 以 `info` 级别记录日志，使用自身的 `status` 和 `code` 返回。
3. **意外错误** -- 以 `error` 级别记录日志，掩盖为 500 `server_error`。
...
`jsonError` 辅助函数位于 [src/server/errors.ts:50-63](https://github.com/Ahoo-Wang/GodeX/blob/main/src/server/errors.ts#L50)，生成标准 JSON 信封：
...
提供商特定的映射（`providerErrorToHttp`）将上游 HTTP 状态码转换为面向客户端的等效状态码，定义于 [src/server/errors.ts:20-44](https://github.com/Ahoo-Wang/GodeX/blob/main/src/server/errors.ts#L20)。

## 通过 toLogEntry 进行结构化日志记录

每个 `GodeXError` 都可以通过 `toLogEntry()`（[src/error/godex-error.ts:24-34](https://github.com/Ahoo-Wang/GodeX/blob/main/src/error/godex-error.ts#L24)）生成可序列化的日志条目。[第 37 行](https://github.com/Ahoo-Wang/GodeX/blob/main/src/error/godex-error.ts#L37)的独立 `toLogEntry(err)` 重载函数可以优雅地处理非 GodeXError 值，将其包装在普通对象中。
...
- [配置模式](../07-configuration/config-schema.md) -- 会话深度限制
