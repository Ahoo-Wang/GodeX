---
title: 会话存储
description: ResponseSessionStore 实现持久化 GodeX 响应快照，使得 previous_response_id 链可以在后续被解析，从而在无状态 HTTP 边界上实现多轮对话。
---

# 会话存储

GodeX 实现了 OpenAI Responses API 的 `previous_response_id` 机制，允许调用者将多个请求链接为一次对话，而无需手动重放消息历史。为此，每个生成的响应必须与其请求快照一起持久化，以便后续请求可以遍历父指针链并重建完整的对话上下文。`ResponseSessionStore` 就是实现这一功能的存储接口，提供了两种实现：用于测试和单进程部署的内存存储，以及用于生产持久化的 SQLite 后端存储。

存储边界被刻意设计得很窄——它持久化的是 API 形状的快照（而非特定提供商的聊天消息），并将链遍历委托给共享的 `resolveResponseSessionChain` 函数。这使得存储层不需要了解特定提供商的协议细节。
...
| `SQLiteResponseSessionStore` | [sqlite.ts:36-149](https://github.com/Ahoo-Wang/GodeX/blob/main/src/session/sqlite.ts#L36) | 使用 WAL 模式的 SQLite 后端存储 |
...
| `createResponseSessionStore` | [session-store-factory.ts:8-16](https://github.com/Ahoo-Wang/GodeX/blob/main/src/context/session-store-factory.ts#L8) | 从配置创建存储的工厂函数 |
...
| 字段 | 类型 | 描述 |
|---|---|---|
| `id` | `ResponseId` | 响应 ID，后续请求用作 `previous_response_id` |
| `previous_response_id` | `ResponseId?` | 父指针，形成轮次的链表结构 |
| `conversation_id` | `ConversationId?` | 保留用于未来 Conversation API 兼容 |
...
| `response` | `StoredResponseSnapshot` | 生成的响应的快照 |
| `metadata` | `Record<string, unknown>?` | 可选的用户提供的元数据 |
...
用于历史记录和诊断的响应数据（[types.ts:59-66](https://github.com/Ahoo-Wang/GodeX/blob/main/src/session/types.ts#L59)）：`id`、`output`、`output_text`、`usage`、`error`、`incomplete_details`。
...
- **读写时克隆**：每次 `get()` 和 `save()` 调用都通过 `cloneStoredResponseSession`（[snapshot-clone.ts:4-7](https://github.com/Ahoo-Wang/GodeX/blob/main/src/session/snapshot-clone.ts#L4)）处理，该函数使用 `structuredClone` 防止调用者通过持有的引用修改已持久化的状态。
- **构造器初始化**：接受可选的初始会话数组，用于测试设置（[memory.ts:22-25](https://github.com/Ahoo-Wang/GodeX/blob/main/src/session/memory.ts#L22)）。
- **清除方法**：暴露 `clear()` 用于测试清理（[memory.ts:63-65](https://github.com/Ahoo-Wang/GodeX/blob/main/src/session/memory.ts#L63)）。
...
数据库模式通过 `migrateResponseSessionSchema`（[sqlite-schema.ts:3-23](https://github.com/Ahoo-Wang/GodeX/blob/main/src/session/sqlite-schema.ts#L3)）自动迁移，并在 `previous_response_id` 和 `conversation_id` 上创建索引以加速链查询。
...
`assertCanSaveSession`（[save-policy.ts:13-40](https://github.com/Ahoo-Wang/GodeX/blob/main/src/session/save-policy.ts#L13)）强制执行两个守卫条件：

| 守卫 | 条件 | 错误码 |
|---|---|---|
| 父指针不匹配 | `expected_previous_response_id` 与会话的 `previous_response_id` 不匹配 | `SESSION_CONFLICT` |
| 覆盖防护 | 已存在记录且 `overwrite` 不为 `true` | `SESSION_CONFLICT` |

两者都抛出带有 `SESSION_CONFLICT` 错误码的 `SessionError`（[codes.ts:43](https://github.com/Ahoo-Wang/GodeX/blob/main/src/error/codes.ts#L43)）。
...
|---|---|
| `"sqlite"` | `SQLiteResponseSessionStore`，使用 `config.sqlite.path` 或默认路径 |
...
flowchart TD
    ...
    path -->|Yes| sqlite_path["SQLiteResponseSessionStore<br>(自定义路径)"]
    path -->|No| sqlite_default["SQLiteResponseSessionStore<br>(默认路径)"]
...
- [链解析](./chain-resolution.md) -- `resolveChain` 如何遍历父指针并构建 `input_items`
