...
搭建一个可用的 GodeX 网关不到五分钟。本指南将引导你完成安装、通过 `godex init` 向导进行交互式配置、启动服务器，以及发送你的第一个 Responses API 请求。完成后，你将拥有一个运行中的网关，能够将 OpenAI Responses API 调用转换为任何内置提供商的 Chat Completions 请求。
...
| 提供商 API 密钥 | DeepSeek、Zhipu 或 MiniMax 中至少一个 |
...
| 4. 测试 | `curl localhost:5678/health` | 验证服务器是否正常 |
...
你也可以使用 Homebrew 安装，或直接从 [GitHub Releases](https://github.com/Ahoo-Wang/GodeX/releases) 下载二进制文件。所有安装方式请参阅 [安装与设置](./installation-setup.md)。
...
运行交互式初始化向导。它会提示你选择提供商、输入 API 密钥、选择会话后端和日志级别，然后写入一个完整的 `godex.yaml`（[src/cli/init/run.ts:8-22](https://github.com/Ahoo-Wang/GodeX/blob/main/src/cli/init/run.ts#L8-L22)）。

```bash
godex init
```

向导使用 `@clack/prompts` 收集以下信息：

| 提示项 | 说明 | 默认值 |
|---|---|---|
| 默认提供商 | 模型前缀不明确时使用的提供商 | `deepseek` |
| API 密钥 | 每个所选提供商的 Bearer 令牌 | （来自环境变量） |
| Base URL | 覆盖每个提供商的端点 | 提供商默认值 |
| 会话后端 | `memory` 或 `sqlite` | `memory` |
| 日志级别 | `trace`、`debug`、`info`、`warn`、`error` | `info` |
| 端口 | 服务器监听端口 | `5678` |
| 配置路径 | `godex.yaml` 的写入位置 | `./godex.yaml` |
...
生成的 `godex.yaml` 类似如下（API 密钥以环境变量引用的形式呈现）：
...
YAML 构建器在 [src/cli/init/config-yaml.ts:6-53](https://github.com/Ahoo-Wang/GodeX/blob/main/src/cli/init/config-yaml.ts#L6-L53) 中组装此结构，并将文件权限设置为 `0600` 以保护 API 密钥。
...
`serve` 命令加载配置、注册内置提供商、创建 `ApplicationContext`，并启动 Bun 的 HTTP 服务器（[src/cli/serve.ts:12-47](https://github.com/Ahoo-Wang/GodeX/blob/main/src/cli/serve.ts#L12-L47)）。
...
你也可以在请求体中添加 `"stream": true` 来流式传输响应。

## 下一步

| 主题 | 说明 |
|---|---|
| [配置](./configuration.md) | 完整的 `godex.yaml` 参考及所有配置项 |
| [内置提供商](./builtin-providers.md) | 比较 DeepSeek、Zhipu 和 MiniMax 的功能 |
| [安装与设置](./installation-setup.md) | Docker、从源码构建和平台二进制文件 |
