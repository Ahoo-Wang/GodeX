---
title: 内置提供商
description: DeepSeek、Zhipu 和 MiniMax 内置提供商的比较，包括各自的功能、默认端点和协议相关的钩子。
---

# 内置提供商

GodeX 内置了三个提供商，覆盖了最流行的非 OpenAI LLM 平台。每个提供商都是一个自包含的模块，声明自身功能、通过钩子转换请求，并将响应映射回标准的 Chat Completions 访问器。添加新提供商遵循相同的模式 -- 实现一个 `ProviderSpec`，编写请求补丁和响应规范化的钩子，然后将其注册到 `Registrar`。
...
| **默认 Base URL** | `api.deepseek.com` | `open.bigmodel.cn`（编程套餐） | `api.minimaxi.com/v1` |
...
每个提供商遵循相同的结构模式：`spec.ts` 声明功能并创建 `ProviderSpec`，`hooks.ts` 实现请求补丁和响应/流访问器，`client.ts` 创建用于发起 HTTP 调用的 `ProviderEdge`，`protocol/` 目录包含提供商特定的 DTO 类型。
...
所有提供商在启动时通过 `createBuiltinRegistrar` 注册（[src/providers/builtin.ts:49-55](https://github.com/Ahoo-Wang/GodeX/blob/main/src/providers/builtin.ts#L49-L55)），该函数创建一个 `Registrar` 并注册每个 `ProviderDefinition`。

## 工具功能比较

每个提供商声明它支持哪些工具类型，以及哪些类型必须被**降级**为更简单的形式。降级意味着 GodeX 在将不支持的工具类型发送给提供商之前，会自动将其转换为最接近的兼容类型。
...
每个提供商以不同方式处理推理（思维链）。桥接内核中的兼容性方案将传入的 `reasoning_effort` 映射为提供商特定的表示。

| 提供商 | 努力程度类型 | 行为 |
|---|---|---|
| DeepSeek | `native` | 映射 `high` -> `high`，`xhigh` -> `max`。在请求中添加 `thinking: {type: "enabled"}`。 |
| Zhipu | `boolean` | 检测到推理内容时添加 `thinking: {type: "enabled", clear_thinking: false}`。 |
| MiniMax | `none` | 完全移除 `reasoning_effort`；不支持推理。 |

DeepSeek 的 `deepSeekPatchRequest` 在 [src/providers/deepseek/hooks.ts:113-136](https://github.com/Ahoo-Wang/GodeX/blob/main/src/providers/deepseek/hooks.ts#L113-L136) 中处理此映射，Zhipu 的等效实现在 [src/providers/zhipu/hooks.ts:113-134](https://github.com/Ahoo-Wang/GodeX/blob/main/src/providers/zhipu/hooks.ts#L113-L134)。
...
每个提供商被包装在一个 `ProviderDefinition` 中，将提供商名称与工厂函数配对。这些定义被收集到 `BUILTIN_PROVIDER_DEFINITIONS` 中并在启动时注册（[src/providers/builtin.ts:22-41](https://github.com/Ahoo-Wang/GodeX/blob/main/src/providers/builtin.ts#L22-L41)）。
...
`ProviderDefinition` 接口定义在 [src/providers/definition.ts:6-11](https://github.com/Ahoo-Wang/GodeX/blob/main/src/providers/definition.ts#L6-L11)，需要一个 `name` 和一个 `create` 工厂函数，该函数从 `ProviderRuntimeConfig` 生成 `ProviderEdge`。

## 提供商规格

### DeepSeek

DeepSeek 规格面向 `https://api.deepseek.com` 的标准 Chat Completions API（[src/providers/deepseek/spec.ts:24-54](https://github.com/Ahoo-Wang/GodeX/blob/main/src/providers/deepseek/spec.ts#L24-L54)）。
...
Zhipu 规格默认使用编程套餐端点 `https://open.bigmodel.cn/api/coding/paas/v4`（[src/providers/zhipu/spec.ts:24-57](https://github.com/Ahoo-Wang/GodeX/blob/main/src/providers/zhipu/spec.ts#L24-L57)）。
...
MiniMax 规格面向 `https://api.minimaxi.com/v1`（[src/providers/minimax/spec.ts:24-54](https://github.com/Ahoo-Wang/GodeX/blob/main/src/providers/minimax/spec.ts#L24-L54)）。
...
| [快速开始](./quick-start.md) | 安装并发起第一次调用 |
...
- [src/providers/deepseek/hooks.ts:18-57](https://github.com/Ahoo-Wang/GodeX/blob/main/src/providers/deepseek/hooks.ts#L18-L57) - DeepSeek 功能和钩子
...
- [src/providers/zhipu/hooks.ts:16-69](https://github.com/Ahoo-Wang/GodeX/blob/main/src/providers/zhipu/hooks.ts#L16-L69) - Zhipu 功能和钩子
...
- [src/providers/minimax/hooks.ts:17-54](https://github.com/Ahoo-Wang/GodeX/blob/main/src/providers/minimax/hooks.ts#L17-L54) - MiniMax 功能和钩子
