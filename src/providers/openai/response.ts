...
import type { ResponseItem, ResponseObject } from "../../protocol/openai/responses";
...
export function buildResponseObject(ctx: ResponsesContext, openAIRes: ChatCompletion): ResponseObject {
	const choice = openAIRes.choices[0];
	const output = buildOutputItems(ctx, openAIRes);
	return buildOpenAIResponseObject(ctx, openAIStatusFields(choice?.finish_reason), {
		output,
...
function buildOutputItems(ctx: ResponsesContext, openAIRes: ChatCompletion): ResponseItem[] {
	const choice = openAIRes.choices[0];
	...
	if (message?.tool_calls && message.tool_calls.length > 0) {
		output.push({
			id: `msg_${ctx.responseId}`, type: "message", role: "assistant", status: "completed",
...
function extractOutputText(output: ResponseItem[]): string {
	return output
		.filter((item): item is Extract<ResponseItem, { type: "message"; content: unknown[] }> =>
