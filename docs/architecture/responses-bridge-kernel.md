...
- Web search uses a hybrid path: providers with native web search receive provider-native declarations; other providers can receive an internal GodeX-managed `web_search` function when `web_search.mode` allows hosted execution. If no backend is configured, `web_search.on_unavailable` controls whether GodeX returns a client-visible function call, rejects the request, or ignores the tool with diagnostics.
